...
const AIQuestionsPanel = ({ categories }) => {
  const [selectedCategory, setSelectedCategory] = useState(
    // ?. → safely first element access
    categories?.[0] ?? null
  );

  const { data, loading, fn: generateFn, } = useFetch(generateInterviewQuestions);   //call api from aiQuestion.js
  ...
  return (
    <div className='flex flex-col gap-4 h-screen overflow-hidden p-3'>
      ...
      <div className='flex flex-wrap gap-1.5'>
        {categories?.map((cat) => (
          <button key={cat} type="button" onClick={() => setSelectedCategory(cat)} className={`text-xs px-3 py-1.5 rounded-lg border transition-all ${selectedCategory === cat
      ...
      <Button variant="gold" size="sm" disabled={loading || !selectedCategory} onClick={() => generateFn({ category: selectedCategory })} className="self-start gap-2">
        {loading ? (
          <>
